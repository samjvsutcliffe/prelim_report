...
for name,s in zip(["200"],scale):
    ...
    print("{}: {}".format(name,ice_length-stress_pos[-1]))
    name = "MPM"
    max_stress *= 1e-6
    #lengths /= s
    ...
    plt.figure(2)
    plt.plot(lengths,ice_length-stress_pos,"-o",label=name)
...
for frame,i in enumerate(bindings):
    ...
    ax.set_xlim([ice_length-1000,ice_length+50])
    ax.set_ylim([-210,40])
